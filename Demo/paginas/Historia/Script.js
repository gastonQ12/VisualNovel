...
if (localStorage.getItem('codigoPartida') !== null) {
    if (new URL(document.referrer).pathname !== '/Demo/index.html') {
        ...
        if (localStorage.getItem('partidasGuardadasLista') !== null ) {
            ...
            let partidaIndividual = []
            const partidasGuardadas = {
                arrayPartida: "",
            };

            Partida.fechaCreacion = new Date().toISOString();
            Partida.partida = localStorage.getItem('codigoPartidaActual');
            partidaIndividual.push(Partida);
            partidasGuardadas.arrayPartida += JSON.stringify(partidaIndividual);
            var listaGuardadosPartidas = localStorage.getItem('partidasGuardadasLista');
            listaGuardadosPartidas += JSON.stringify(partidasGuardadas)
            localStorage.setItem('partidasGuardadasLista', listaGuardadosPartidas);
        }
        /*
